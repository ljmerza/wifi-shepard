...
**v1 in progress** — the core daemon is implemented and tested (scanner → scorer → actor, dry-run gate, UniFi backend, SQLite, SIGHUP/SIGTERM + config file-watch reload); a UI sidecar (ADR-0002) ships alongside it with read-only history/overview pages, **a Settings page (ADR-0013) that edits the whole config** — env-var-reference secrets, daemon-validated saves, hot-reload — and **per-device editing (ADR-0014)**: an allowlist toggle on every devices-list row plus a card on `/devices/{mac}` for that device's overrides, inactivity opt-in, and reboot eligibility. See [`docs/adr/0000-adr-index.md`](./docs/adr/0000-adr-index.md) for what's shipped vs. still open.
...
│                                 #   device_config.py + POST /devices/{mac}/settings (ADR-0014
│                                 #   per-device surgical edits; _fields.html shares the controls)
...
- Allowlist (`allowlist:`) — MACs in this list are never kicked. Toggle it per device from the devices table or `/devices/{mac}` (ADR-0014), or edit the list wholesale on the Settings page.
